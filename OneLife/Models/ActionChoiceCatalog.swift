...
enum ActionChoiceCatalog {
    static func definition(for choiceID: ActionChoiceID) -> ActionChoiceDefinition {
        ...
        // CE2: Dedicated Crime Actions (Legitimacy vs Lethality tension)
        case .crimeLayLow:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Lay Low", subtitle: "Drop heat, lose momentum.", identityLine: "You trade momentum for oxygen.", previewTags: ["Heat drops", "Cash stops", "Safe"], preferredEventTags: ["crime": 5, "risk": 2], microBeat: "Off the grid.", baseFriction: .none)
        case .crimeRecruitAssociate:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Recruit Muscle", subtitle: "Loyalty at a cost.", identityLine: "You bring in outsiders. The circle widens, but so does the risk.", previewTags: ["Loyalty up", "Legitimacy down", "Heat up"], preferredEventTags: ["crime": 6, "social": 3], microBeat: "A new face at the table.", baseFriction: .resistance)
        case .crimeLaunderFunds:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Wash Cash", subtitle: "Slower money, safer money.", identityLine: "You turn the dirty money clean. It costs you on the margin, but buys you sleep.", previewTags: ["Legitimacy up", "Margins down", "Heat down"], preferredEventTags: ["crime": 4, "finance": 6], microBeat: "Cook the books.", baseFriction: .warning)
        case .crimeCutTies:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Cut Ties", subtitle: "A brutal, clean exit.", identityLine: "You sever the connection before it can burn you.", previewTags: ["Heat spikes", "Exit lane", "Dangerous"], preferredEventTags: ["crime": 8, "risk": 6], microBeat: "Burn the bridge.", baseFriction: .danger)
        ...
        case .startCoachingCareer:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Become Program Coach", subtitle: "Special Tier — Build a program legacy (peak athlete or college coaching cred + capital required)", identityLine: "You decide the next game is not played with your body. It is played through the people you recruit, the system you install, and the boosters you manage — or survive.", previewTags: ["Special", "Program", "Pressure", "Legacy"], preferredEventTags: ["career": 9, "social": 7, "risk": 7, "fame": 5], microBeat: "The whistle hangs differently. So does the weight of the program.", baseFriction: .warning)
        case .recruitTalent:
        ...
        case .startSportsOwnership:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Become Sports Owner", subtitle: "Diamond Tier — Buy major-league franchises ($2B+ wealth + elite sports/business cred required)", identityLine: "You decide the next game is not coached from the sideline. It is owned from the boardroom — media deals, valuations, and billion-dollar bets on teams that print money when you run them right.", previewTags: ["Diamond", "Empire", "Franchise", "Capital risk", "Portfolio"], preferredEventTags: ["career": 9, "money": 9, "fame": 6, "risk": 6], microBeat: "The league office takes your call now.", baseFriction: .warning)
        case .acquireFranchise:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Acquire Franchise", subtitle: "Buy a major-league team.", identityLine: "You decide another city’s team belongs in your portfolio — and you have the capital to prove it.", previewTags: ["Portfolio +", "Billions", "Valuation"], preferredEventTags: ["money": 9, "career": 7], microBeat: "The sale closes at league headquarters.", baseFriction: .warning)
        case .hireGeneralManager:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Hire General Manager", subtitle: "Upgrade the front office.", identityLine: "You decide the football stays off your desk — but the people running the desk need to be elite.", previewTags: ["Front office +", "Cash cost", "Margins"], preferredEventTags: ["career": 7, "money": 5], microBeat: "The org chart gets sharper.", baseFriction: .resistance)
        case .negotiateMediaDeal:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Negotiate Media Deal", subtitle: "Lock in broadcast revenue.", identityLine: "You decide the real game is the contract that pays whether the team wins or not.", previewTags: ["Media +", "Revenue", "Leverage"], preferredEventTags: ["money": 8, "career": 6], microBeat: "The lawyers stay up all night.", baseFriction: .resistance)
        case .investInBrand:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Invest In Brand", subtitle: "Grow fan loyalty and equity.", identityLine: "You decide the jersey, the stadium experience, and the story are assets — not decorations.", previewTags: ["Brand +", "Fans +", "Cash cost"], preferredEventTags: ["career": 6, "money": 5, "fame": 4], microBeat: "The rebrand lands.", baseFriction: .none)
        case .expandPortfolio:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Expand Portfolio", subtitle: "Add another franchise.", identityLine: "You decide one team was never going to be enough.", previewTags: ["Portfolio +", "Billions", "Diversification"], preferredEventTags: ["money": 9, "career": 7], microBeat: "Another market, another trophy case.", baseFriction: .warning)
        case .sellFranchise:
            return ActionChoiceDefinition(choiceID: choiceID, title: "Sell Franchise", subtitle: "Cash out a team at valuation.", identityLine: "You decide liquidity beats nostalgia — at least for one franchise.", previewTags: ["Cash in", "Portfolio -", "Exit"], preferredEventTags: ["money": 8, "career": 5], microBeat: "The check clears.", baseFriction: .resistance)
        case .runScheme:
